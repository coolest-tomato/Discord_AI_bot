...
- Be honest and direct. If the writing is off-topic, clearly state it is off-topic and explain why.
- Do NOT praise off-topic or irrelevant writing. Be strict but constructive.
**문법 (Grammar)**
...
async def daily_writing_alarm():
    ...
    # ✅ 이 부분 추가 - 채널 멤버 전체를 작문 대기 모드로 설정
    async for member in channel.guild.fetch_members():
        if not member.bot:
            user_sessions[member.id] = {
                "topic": topic_data,
...
async def free_mode(ctx):
    """자유 대화 모드 시작"""
    bot.sleep_mode = False
    # 자유 대화 모드로 세션 설정
    user_sessions[ctx.author.id] = {"chat_mode": True}
    await ctx.send("😊 자유 대화 모드예요! 편하게 말 걸어주세요.")
...
async def help_command(ctx):
    ...
    embed.add_field(name="!stop", value="도우미 휴면상태 돌입", inline=False)
    embed.add_field(name="!free", value="도우미 휴면상태에서 깨워서 대화하기", inline=False)
